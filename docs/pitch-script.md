...
1. *Here on the **Overview**, you see our primary benchmark metric: **Incremental Recovered Revenue (ΔR)**. In this demo session, REVIVE recovered substantial revenue at risk while eliminating friction on permanent bank declines.*
2. *Let’s drill into **Recovery Cases**:*
   - *Look at Case **txn_00000104**: A transient UPI timeout. REVIVE diagnosed it with 95% confidence, determined `RETRY` had the highest expected value (+INR 2,204), passed all safety gates, and when we click **Execute**, it recovers the funds immediately.*
   - *Now look at Case **txn_00000219**: A high-risk account. Instead of auto-executing, Rule **P005** instantly intercepted it and routed it to **Human Review**. The execution button is disabled. Automated intervention on high-risk accounts is completely blocked.*
   - *And if we look at Case **txn_00000305**: The payment had already received 2 previous attempts. Rule **P003** strictly halted further automation to prevent duplicate billing."*
...
- *The unconstrained **`RULE_BASED`** strategy actually achieved higher gross synthetic recovery (INR 3,626,512) by aggressively intervening on every single failure.*
- *We deliberately did **not** optimize REVIVE to maximize unconstrained gross revenue at all costs.*
- ***REVIVE is a governed recovery system**: It recovered **INR 2,726,857.62 (75.2% of the rule-based gross revenue)** while using **378 fewer interventions per 10k transactions—a 22.4% reduction in interventions**.*
- *Against the passive baseline, REVIVE delivered **INR 2,655,515.22 in incremental revenue (ΔR)** with **56.04% precision** (compared to just 30.84% for naive retries), while escalating 624 ambiguous cases to human review and allowing zero automated high-risk leaks.*
